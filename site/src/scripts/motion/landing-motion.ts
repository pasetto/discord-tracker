...
export function resolveHeroEntranceTargets(root: ParentNode): HeroEntranceTargets {
  ...
  const brand = Array.from(hero?.querySelectorAll('[data-motion="hero-brand"]') ?? []);
  return {
    brand,
...
export function playModeToggleTransition(
  ...
  const headline =
    opts.root.querySelector('[data-testid="landing-hero-mock-headline"]') ??
    opts.root.querySelector('[data-motion="mock-body"]') ??
    opts.root;
...
export function observeScrollReveals(
  ...
  const sections = SCROLL_REVEAL_SECTION_SELECTORS.map((sel) => root.querySelector(sel)).filter(
...
/**
 * Lazy-import do anime.js (só no client da landing).
