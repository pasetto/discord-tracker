...
function createAnimeMock(): AnimeMotionApi & {
  animate: jasmine.Spy;
  createTimeline: jasmine.Spy;
  stagger: jasmine.Spy;
} {
  const timeline = {
    add: jasmine.createSpy('timeline.add').and.callFake(function (this: unknown) {
      return this;
    }),
    call: jasmine.createSpy('timeline.call').and.callFake(function (this: unknown) {
      return this;
    }),
    pause: jasmine.createSpy('timeline.pause'),
    cancel: jasmine.createSpy('timeline.cancel'),
  };
  return {
    animate: jasmine.createSpy('animate'),
    createTimeline: jasmine.createSpy('createTimeline').and.returnValue(timeline),
    stagger: jasmine.createSpy('stagger').and.callFake((v: number) => v),
  };
}

describe('landing motion helpers (SYN-107)', () => {
  ...
  it('prefersReducedMotion lê matchMedia reduce', () => {
    expect(
      prefersReducedMotion(() => ({ matches: true }) as MediaQueryList),
    ).toBe(true);
    expect(
      prefersReducedMotion(() => ({ matches: false }) as MediaQueryList),
    ).toBe(false);
  });
  ...
  it('lista seções de scroll reveal da landing', () => {
    expect(SCROLL_REVEAL_SECTION_SELECTORS).toContain(
